...
a.	На R1 настройте приоритет OSPF интерфейса G0/0/1 на 50, чтобы убедиться, что R1 является назначенным маршрутизатором.
    ...
    R1(config-if)#do clea
    R1(config-if)#do clea ip ospf proc
    Reset ALL OSPF processes? [no]: yes
    
    R1(config-if)#
    01:27:15: %OSPF-5-ADJCHG: Process 56, Nbr 2.2.2.2 on GigabitEthernet0/0/1 from FULL to DOWN, Neighbor
    ...
    01:27:20: %OSPF-5-ADJCHG: Process 56, Nbr 2.2.2.2 on GigabitEthernet0/0/1 from LOADING to FULL,
    Loading Done
...
b.	Настройте таймеры OSPF на G0/0/1 каждого маршрутизатора для таймера приветствия, составляющего 30
секунд.

        R1(config-if)#ip ospf hello-interval 30 
        R1(config-if)#do show ip ospf interface GigabitEthernet0/0/1
        ...
        01:31:44: %OSPF-5-ADJCHG: Process 56, Nbr 1.1.1.1 on GigabitEthernet0/0/1 from LOADING to FULL,
        Loading Done
        
        R2(config-if)#do show ip ospf interface GigabitEthernet0/0/1
...
Сошлись все параметры кроме dead
b.	На R1 выполните команду show ip route ospf, чтобы убедиться, что сеть R2 Loopback1 присутствует в таблице маршрутизации. Обратите внимание на разницу в метрике между этим выходным и предыдущим выходным. Также обратите внимание, что маска теперь составляет 24 бита, в отличие от 32 битов, ранее объявленных.
        
        R1(config)#do show ip route ospf
        O 192.168.1.0 [110/10] via 10.53.0.2, 00:05:59, GigabitEthernet0/0/1
c.	Введите команду show ip route ospf на маршрутизаторе R2. Единственная информация о маршруте OSPF должна быть распространяемый по умолчанию маршрут R1.

        R2#show ip route ospf 
        O*E2 0.0.0.0/0 [110/1] via 10.53.0.1, 00:00:14, GigabitEthernet0/0/1
d.	Запустите Ping до адреса интерфейса R1 Loopback 1 из R2. Выполнение команды ping должно быть успешным.
![](https://github.com/EvilPandaKBN/practice/blob/main/HW/HW10/Screen/R2-R1L.png)
#### Вопрос <br>
Почему стоимость OSPF для маршрута по умолчанию отличается от стоимости OSPF в R1 для сети 192.168.1.0/24?
#### Ответ <br>
Потому что маршрут по умолчанию задан вручную, на R1 ему принудительно задается метрика 1. А метрика сети 192.168.1.0/24 рассчитывается по формуле
